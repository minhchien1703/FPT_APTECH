<?php
class App {
    private $__controller, $__action, $__params, $__conn, $controller, $__middelware;

    function __construct($conn, AuthMiddelware $middelware)
    {
        $this->__controller = "home";
        $this->__action = "index";
        $this->__params = [];
        $this->__conn = $conn;
        $this->__middelware = $middelware;
    ...
    public function handleUrl () {
        $path = $this->getPath();
        // !empty check path neu ton tai va khong rong thi true
        if (!empty($path)) {
            ...
            if (method_exists($this->__controller, $this->__action)) {
                $this->__middelware->execute($this->controller, $this->__action);
